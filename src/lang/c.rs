...
impl C {
    ...
    fn out_header(
        &mut self,
        fh: &mut BufWriter<fs::File>,
        fc: &mut BufWriter<fs::File>,
    ) -> Result<(), &'static str> {
    ...
    fn out_footer(&mut self, f: &mut BufWriter<fs::File>) -> Result<(), &'static str> {
        let mut doblock = move || -> Result<(), Box<dyn Error>> {
            ...
            writeln!(
                f,
                "  return (iter_idx >= {me}_sz) ? NULL : &{me}_data[iter_idx++];",
                me = varname
            )?;
            writeln!(f, "}}")?;
            writeln!(f)?;
            ...
            writeln!(
                f,
                "  return (p >= {me}_sz) ? NULL: &{me}_data[p];",
                me = varname
            )?;
            writeln!(f, "}}")?;
            writeln!(f)?;
    ...
    pub fn generate_files(&mut self) -> Result<(), &'static str> {
        let cols = if self.go.hex { 13 } else { 16 };
        ...
        self.go
            .write_data(&mut ofile_c, cols, writeif, String::from("\n"))?;
